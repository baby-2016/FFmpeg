...
# ============================================================
# FFmpeg n7.1 minimal build script — macOS arm64
# Usage:
#   ./build-mac-arm.sh build-dep   # build external deps
#   ./build-mac-arm.sh compile     # configure + compile FFmpeg
# ============================================================

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DEPS_DIR="$SCRIPT_DIR/deps"
INSTALL_PREFIX="$DEPS_DIR/install-mac-arm64"
OUTPUT_DIR="$SCRIPT_DIR/output-mac-arm64"
...
build_fdk_aac() {
    ...
    make distclean || true
    autoreconf -fiv
    CFLAGS="-O2 -arch arm64" CXXFLAGS="-O2 -arch arm64" LDFLAGS="-arch arm64" \
    ./configure \
        --prefix="$INSTALL_PREFIX" \
        --disable-shared \
        --enable-static \
